...
class SimAnneal(object):
    ...
    #    2-opt
        cur_node = random.choice(self.nodes)  # start from a random node for optimization
        solution = [cur_node]

        free_nodes = set(self.nodes)
        free_nodes.remove(cur_node)
        while free_nodes:
            next_node = min(free_nodes, key=lambda x: self.dist(cur_node, x))  #MIN Distance finding
    ...
    def anneal(self):
        ...
        # Initialize with the 2-opt solution.
        ...
        print(f"Improvement over heuristic: {improvement : .2f}%")
